...
namespace GXEBRebackSaveTool.Utils
{
    ...
    class EquipmentHelper
    {
        ...
        private static Dictionary<EquipmentNS, byte[]> HandleAutoReportNS(byte[] dataBody)
        {
            ...
            int PackageType = 0;
            //加入错误包兼容  20180927
            if ((dataBody[0] == 0x81) && (dataBody[32] == 0x05) && (dataBody[38] == 0x00) && (dataBody[39] == 0x4A))
            {

                PackageType = 0x55;//错误包
            }
            else
            {
                datatagetnum = ConvertHelper.Byte2int(dataBody.Skip(12).Take(2).ToArray());
                PackageType = ConvertHelper.Byte2int(dataBody.Skip(12 + 2 + 12 * datatagetnum).Take(1).ToArray());
            }
            switch (PackageType)
            {
                case 0x10:
                ...
                case 0x11:
                    ...
                    break;
                case 0x55:
                    data.Add(EquipmentNS.Heartbeat, dataBody.Skip(33).Take(5).ToArray());
                    break;
            }
            return data;
        }
